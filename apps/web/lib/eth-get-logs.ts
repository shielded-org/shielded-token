...
/** Parse "up to a 10 block range" / "10 block range" from JSON-RPC error text. */
...
/**
 * Descending chunk sizes to try from a caller cap down to 1.
 * When `explicitMax` is set, do not merge huge default windows (that defeated L2 / Alchemy tuning).
 */
...
export function recommendedLogChunkSizeForRpc(netChainId: number, rpcUrl: string, l2ChainIds: readonly number[]): number | undefined {
...
/**
 * Inclusive block range. Bisects on failure so a failing sub-range never drops logs from a successful one.
 */
export async function ethGetLogsRange(
  ...
  try {
    ...
    if (limit != null && toBlock - fromBlock + 1 > limit) {
      ...
      const tail =
        end < toBlock ? await ethGetLogsRange(provider, address, topics, end + 1, toBlock) : [];
      return [...head, ...tail];
    }
...
/**
 * Fetch logs in block chunks. Retries with smaller chunks when the RPC rejects wide ranges.
 */
