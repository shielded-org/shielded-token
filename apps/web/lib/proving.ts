...
function normalizeProofToBytes(raw: unknown): Uint8Array {
  ...
  if (raw && typeof raw === "object" && "proof" in raw) {
    return normalizeProofToBytes((raw as {proof: unknown}).proof);
  }
  throw new Error("Unsupported proof format returned by UltraPlonkBackend");
}
...
export async function loadCircuitArtifact(): Promise<CircuitJson> {
  ...
  if (!res.ok) throw new Error("Missing shielded_transfer.json in web public assets");
  return (await res.json()) as CircuitJson;
}

type ProofInputs = {
  spending_key: `0x${string}`;
  in_amounts: [string, string];
  in_blindings: [`0x${string}`, `0x${string}`];
  merkle_siblings: [`0x${string}`[], `0x${string}`[]];
  merkle_directions: [boolean[], boolean[]];
  out_amounts: [string, string];
  out_recipient_pks: [`0x${string}`, `0x${string}`];
  out_blindings: [`0x${string}`, `0x${string}`];
  token: `0x${string}`;
  merkle_root: `0x${string}`;
  nullifiers: [`0x${string}`, `0x${string}`];
  out_commitments: [`0x${string}`, `0x${string}`];
  fee: string;
  fee_recipient_pk: `0x${string}`;
  mode: string;
  unshield_recipient: `0x${string}`;
  unshield_amount: string;
  unshield_token_address: `0x${string}`;
};
...
export async function generateProof(params: {
  ...
  const inputs: ProofInputs = {
    ...
    unshield_recipient: ethers.zeroPadValue(params.unshieldRecipient ?? "0x00", 32) as `0x${string}`,
    unshield_amount: (params.unshieldAmount ?? 0n).toString(),
    unshield_token_address: ethers.zeroPadValue(params.unshieldTokenAddress ?? "0x00", 32) as `0x${string}`,
  };
  ...
  return {proof: ethers.hexlify(proofBytes) as `0x${string}`};
}
