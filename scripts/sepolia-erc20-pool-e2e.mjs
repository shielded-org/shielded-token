...
async function main() {
  ...
  if (SKIP_DEPLOY) {
    ...
    const pool = await poolFactory.deploy(verifierAddress, treeAddress, deployerAddress, [tokenAddress]);
  ...
  if (tokenBal < 120n) throw new Error(`Deployer token balance too low for scripted shields: ${tokenBal}`);
